    ...
    <div className="space-y-2">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-foreground-secondary">
            {node.isFolder ? (
              ...
              <>
                ...
                {typeof node.pages === "number" ? (
                  <span className="rounded-full border border-indigo-300 dark:border-indigo-500/20 bg-indigo-100 dark:bg-indigo-500/10 px-2 py-0.5 text-indigo-700 dark:text-indigo-200">
                    {node.pages} pages
                  </span>
                ) : null}
...
export default function ZipViewerModal({
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="relative flex h-full w-full flex-col overflow-hidden rounded-none border border-border bg-gradient-to-br from-zinc-900 via-zinc-950 to-black shadow-2xl sm:h-[95vh] sm:w-[96vw] sm:rounded-3xl sm:border-border-strong md:h-[92vh] md:w-[90vw] lg:h-[90vh] lg:w-[85vw] xl:h-[88vh] xl:w-[80vw] 2xl:h-[86vh] 2xl:w-[75vw]"
      >
        ...
        <header className="relative flex items-center justify-between border-b border-border bg-gradient-to-r from-black/40 via-black/30 to-black/40 px-4 py-3 sm:px-6 sm:py-4">
          ...
          <button
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center rounded-lg bg-red-100 dark:bg-red-500/15 text-red-700 dark:text-red-400 transition hover:scale-105 hover:bg-red-100 dark:hover:bg-red-500/25 active:scale-95 sm:h-10 sm:w-10 sm:rounded-xl"
            ...
            <div className="mt-3 text-center sm:mt-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-indigo-300 dark:border-indigo-500/30 bg-indigo-100 dark:bg-indigo-500/15 px-3 py-1.5 sm:px-4 sm:py-2">
                <span className="text-sm text-indigo-700 dark:text-indigo-300">
                  <span className="mr-1 text-indigo-700 dark:text-indigo-400">{currentMessage.icon}</span>
          ...
          {!showPreview ? (
            <div className="flex h-full items-center justify-center">
              <div className="max-w-md px-4 text-center">
                <div className="relative mx-auto mb-6">
                  <div className="h-16 w-16 animate-spin rounded-full border-2 border-indigo-300 dark:border-indigo-500/30 border-t-indigo-500 sm:h-20 sm:w-20" />
                  <div className="absolute inset-0 h-16 w-16 animate-pulse rounded-full bg-indigo-100 dark:bg-indigo-500/10 sm:h-20 sm:w-20" />
                </div>
              ...
              <div className="mx-auto max-w-5xl space-y-5">
                <section className="rounded-2xl border border-border bg-card p-4 sm:p-5">
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="rounded-xl border border-border bg-card p-4">
                      <p className="text-xs uppercase tracking-wide text-foreground-faint">Files</p>
                      <p className="mt-1 text-2xl font-semibold text-foreground">{summary.files}</p>
                    </div>
                    <div className="rounded-xl border border-border bg-card p-4">
                      <p className="text-xs uppercase tracking-wide text-foreground-faint">Folders</p>
                      <p className="mt-1 text-2xl font-semibold text-foreground">{summary.folders}</p>
                    </div>
                    <div className="rounded-xl border border-border bg-card p-4">
                      <p className="text-xs uppercase tracking-wide text-foreground-faint">PDF Pages</p>
                      <p className="mt-1 text-2xl font-semibold text-foreground">{summary.pdfPages}</p>
                    </div>
                  </div>
                </section>
                ...
                <section className="rounded-2xl border border-border bg-card p-4 sm:p-5">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <h3 className="text-sm font-semibold text-foreground sm:text-base">Folder Structure</h3>
                    <p className="text-xs text-foreground-faint">Client-side archive preview</p>
                  </div>
            ...
            <div className="flex h-full items-center justify-center p-4 sm:p-6">
              <div className="max-w-md text-center">
                ...
                <button
                  onClick={onDownload}
                  className="flex w-full items-center justify-center gap-3 rounded-xl bg-emerald-500 px-8 py-4 font-medium text-foreground transition hover:scale-105 hover:bg-emerald-600 active:scale-95"
                >
