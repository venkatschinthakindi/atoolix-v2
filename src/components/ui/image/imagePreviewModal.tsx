      ...
      >
        ...
        <header className="relative flex items-center justify-between border-b border-border bg-gradient-to-r from-black/40 via-black/30 to-black/40 px-4 py-3 sm:px-6 sm:py-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 id="image-modal-title" className="truncate text-sm font-semibold text-foreground sm:text-base">
                {documentName} {variant === "preview" ? "Preview" : "Download"}
              </h2>
              <p className="truncate text-xs text-foreground-faint sm:text-sm">
          ...
          <div className="flex items-center gap-2">
            {variant === "preview" && (
              <div className="hidden items-center gap-2 sm:flex">
                <button
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-lg bg-card text-foreground transition hover:bg-surface-raised active:scale-95"
                  aria-label="Zoom out"
                >
                  <ZoomOut className="h-4 w-4" />
                </button>
                <span className="min-w-[56px] text-center text-xs font-medium text-foreground-secondary sm:text-sm">
            ...
            {variant === "preview" && (
              <div className="flex items-center gap-1 sm:hidden">
                <button
                  onClick={() => setScale((s) => Math.max(0.2, +(s - 0.1).toFixed(2)))}
                  className="flex h-8 w-8 items-center justify-center rounded-lg bg-card text-foreground"
                  aria-label="Zoom out"
                >
                  −
                </button>
                <span className="min-w-[45px] text-center text-xs text-foreground-secondary">{Math.round(scale * 100)}%</span>
                <button
                  onClick={() => setScale((s) => Math.min(3, +(s + 0.1).toFixed(2)))}
                  className="flex h-8 w-8 items-center justify-center rounded-lg bg-card text-foreground"
                  aria-label="Zoom in"
            ...
            <button
              onClick={onClose}
              className="flex h-9 w-9 items-center justify-center rounded-lg bg-red-100 dark:bg-red-500/15 text-red-700 dark:text-red-400 transition hover:bg-red-100 dark:hover:bg-red-500/25 active:scale-95 sm:h-10 sm:w-10 sm:rounded-xl"
          ...
          <div className="rounded-2xl border border-border bg-surface-sunken p-3">
            ...
            {isPreparing && !showPreview && (
              <div className="mt-3 text-center">
                <div className="inline-flex items-center gap-2 rounded-full border border-indigo-300 dark:border-indigo-500/30 bg-indigo-100 dark:bg-indigo-500/15 px-3 py-1.5 text-sm text-indigo-700 dark:text-indigo-300">
            ...
            <div className="flex h-full items-center justify-center px-4">
              <div className="w-full max-w-md text-center">
                <div className="mx-auto mb-6 relative h-16 w-16 sm:h-20 sm:w-20">
                  <div className="absolute inset-0 animate-spin rounded-full border-2 border-indigo-300 dark:border-indigo-500/30 border-t-indigo-500" />
                  <div className="absolute inset-0 animate-pulse rounded-full bg-indigo-100 dark:bg-indigo-500/10" />
                </div>
                  ...
                  {loadingMessages.map((_, index) => (
                    <div
                      ...
                      className={`h-2 w-2 rounded-full transition-all duration-300 ${
                        index <= loadingStep ? "scale-125 bg-indigo-500" : "bg-surface-raised"
                      }`}
                    />
                  ))}
                </div>
                ...
                <div className="mx-auto mt-6 h-1.5 w-full max-w-[220px] overflow-hidden rounded-full bg-surface-raised">
                  <div
            ...
            <div className="flex h-full items-center justify-center p-4 sm:p-6">
              <div className="w-full max-w-md text-center">
                <div className="mb-6 flex items-center justify-center">
                  <div className="flex h-24 w-24 items-center justify-center rounded-full bg-emerald-100 dark:bg-emerald-500/20 animate-pulse">
                    <CheckCircle2 className="h-14 w-14 text-emerald-700 dark:text-emerald-400" />
                  </div>
                </div>

                <h3 className="mb-2 text-xl font-semibold text-foreground">Your image is ready</h3>
                <p className="mb-2 text-sm text-foreground-secondary">{documentName}</p>
                <p className="mb-6 text-sm text-foreground-faint">Everything is prepared. Click below to download.</p>
                ...
                <p className="mt-4 text-xs text-foreground-faint">Secure download • Local processing • Instant access</p>
              </div>
            </div>
          )}
        </main>
        ...
        {variant === "preview" && (
          <footer className="relative border-t border-border bg-surface-sunken px-4 py-2 text-center text-xs text-foreground-faint sm:px-6 sm:text-sm">
            Keyboard: <kbd className="rounded bg-surface-raised px-1.5 py-0.5 font-mono">Esc</kbd> to close,{" "}
            <kbd className="rounded bg-surface-raised px-1.5 py-0.5 font-mono">+</kbd> /{" "}
            <kbd className="rounded bg-surface-raised px-1.5 py-0.5 font-mono">-</kbd> to zoom
          </footer>
        )}
      </div>
    </div>
