...
export default function InstallButton() {
  ...
  return (
    ...
    >
      {/* wide tooltip card — sits above the handle, not beside it */}
      <div
        role="tooltip"
        className={`
          pointer-events-none absolute bottom-full right-3 mb-3 w-[250px]
          rounded-2xl border border-white/[0.18] bg-indigo-950/80 p-4
          shadow-[0_24px_60px_-20px_rgba(0,0,0,0.65)] backdrop-blur-2xl
          transition-all duration-300 ease-out
          ${showTip && !expanded && !dragging ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0"}
        `}
      >
      ...
      <div
        ...
        className="
          relative flex items-stretch overflow-hidden rounded-l-[20px]
          border border-white/[0.18] border-r-0
          bg-gradient-to-b from-white/[0.14] to-white/[0.05]
          shadow-[0_25px_60px_-18px_rgba(0,0,0,0.55)] backdrop-blur-2xl
        "
      >
        {/* hairline sheen along the top */}
        ...
        {/* handle — this stays on screen permanently */}
        <button
          type="button"
          onClick={toggle}
          onMouseEnter={() => setShowTip(true)}
          onMouseLeave={() => setShowTip(false)}
          aria-label={expanded ? "Collapse install panel" : "Expand install panel"}
          aria-expanded={expanded}
          className="group relative flex w-[34px] shrink-0 cursor-grab flex-col items-center justify-center gap-2.5 border-r border-white/[0.16] py-4 active:cursor-grabbing"
        >
          <span className="pointer-events-none absolute inset-0 bg-[#F1DBA0]/0 transition-colors duration-300 group-hover:bg-[#F1DBA0]/[0.08]" />
        ...
        <div className="flex flex-1 flex-col">
          <div className="flex flex-1 items-center gap-3 px-4 py-3.5">
            ...
            {/* collapses back to the edge tab — does not hide the feature */}
            <button
              type="button"
              onClick={collapse}
              aria-label="Collapse to edge"
              className="shrink-0 rounded-full p-1.5 text-white/40 transition-colors hover:bg-white/[0.12] hover:text-white/85"
            >
          ...
          {/* benefit strip — informative, not decorative */}
          ...
          <button
            ...
            className="
              ...
              border-t border-white/[0.16] bg-gradient-to-r from-[#F1DBA0] to-[#C9A15C]
              py-3 text-[12.5px] font-semibold tracking-tight text-[#141414]
              transition-transform active:scale-[0.98]
            "
          >
            <span className="pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/40 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
            <svg viewBox="0 0 24 24" className="relative size-3.5" fill="none">
              <path d="M12 5v9M12 14l-3.2-3.2M12 14l3.2-3.2" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
