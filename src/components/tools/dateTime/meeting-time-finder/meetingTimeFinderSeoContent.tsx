  ...
  return (
    <div className="mx-auto space-y-8 px-3 py-5 text-white sm:px-4 sm:py-6 lg:px-5 lg:py-8">
      ...
      <section aria-labelledby="intro-heading" className="space-y-3">
        <p className="inline-flex rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-medium text-white/70">Free Online Meeting Time Finder</p>
        ...
        <p className="max-w-4xl text-sm leading-7 text-white/75 sm:text-[0.95rem]">Find a meeting time that works across multiple time zones without manually converting every participant's local time. Search for locations by city, country, or time zone, set local working hours for each location, choose a meeting duration, and find upcoming meeting slots where everyone is available.</p>
        <p className="max-w-4xl text-sm leading-7 text-white/65 sm:text-[0.95rem]">The Meeting Time Finder accounts for daylight saving time and date-specific time zone offsets. You can use meeting templates, add a title and description, share the complete setup, export meeting data as CSV, or download the selected meeting as an .ics calendar event.</p>
      </section>
      <section aria-labelledby="features-heading" className="space-y-4"><SectionHeading id="features-heading" title="What the Meeting Time Finder Can Do" description="Find practical meeting slots using time zones, working hours, meeting duration, and upcoming availability." /><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{featureItems.map((item) => <article key={item.title} className="rounded-xl border border-white/10 bg-white/5 p-4"><div className="text-lg" aria-hidden="true">{item.icon}</div><h3 className="mt-2 text-sm font-semibold sm:text-[0.95rem]">{item.title}</h3><p className="mt-1.5 text-sm leading-6 text-white/70">{item.desc}</p></article>)}</div></section>
      <section aria-labelledby="how-heading" className="space-y-4"><SectionHeading id="how-heading" title="How to Find a Meeting Time Across Time Zones" description="Set the participants' locations and schedules, then let the finder search for overlapping availability." /><ol className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{howToSteps.map((step, index) => <li key={step.title} className="rounded-xl border border-white/10 bg-white/5 p-4"><div className="flex items-center gap-2"><span className="text-lg" aria-hidden="true">{step.icon}</span><span className="text-xs font-semibold text-white/50">Step {index + 1}</span></div><h3 className="mt-2 text-sm font-semibold">{step.title}</h3><p className="mt-1.5 text-sm leading-6 text-white/70">{step.desc}</p></li>)}</ol></section>
      <section aria-labelledby="templates-heading" className="space-y-4"><SectionHeading id="templates-heading" title="Meeting Templates" description="Start with a practical duration or choose a custom meeting length." /><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{meetingTemplates.map((item) => <article key={item.title} className="rounded-xl border border-white/10 bg-white/5 p-4"><h3 className="text-sm font-semibold">{item.title}</h3><p className="mt-1 text-xs text-white/50">{item.duration}</p><p className="mt-2 text-sm leading-6 text-white/70">{item.desc}</p></article>)}</div></section>
      <section aria-labelledby="examples-heading" className="space-y-4"><SectionHeading id="examples-heading" title="Meeting Time Examples" description="Common ways to use the finder when participants work across regions." /><div className="grid gap-3 md:grid-cols-3">{meetingExamples.map((item) => <article key={item.title} className="rounded-xl border border-white/10 bg-white/5 p-4"><h3 className="text-sm font-semibold">{item.title}</h3><p className="mt-2 text-sm leading-6 text-white/70">{item.desc}</p></article>)}</div></section>
      <section aria-labelledby="use-cases-heading" className="space-y-4"><SectionHeading id="use-cases-heading" title="Meeting Time Finder Use Cases" description="Useful for teams, clients, interviews, events, and international scheduling." /><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">{useCases.map((item) => <article key={item.title} className="rounded-xl border border-white/10 bg-white/5 p-4"><h3 className="text-sm font-semibold">{item.title}</h3><p className="mt-2 text-sm leading-6 text-white/70">{item.desc}</p></article>)}</div></section>
