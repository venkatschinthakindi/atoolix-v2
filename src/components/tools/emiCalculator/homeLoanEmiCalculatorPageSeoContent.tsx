  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 p-4 text-foreground">
      ...
      <section className="space-y-4" aria-labelledby="home-loan-intro">
        ...
        <p className="text-sm leading-7 text-foreground-secondary">A home loan EMI depends mainly on the loan amount, annual interest rate, and repayment tenure. Use the calculator to compare monthly payments and understand how changing the rate or tenure affects total repayment.</p>
        <p className="text-sm leading-7 text-foreground-secondary">Home loans often run for many years, so a small change in interest rate or tenure can materially change the total interest paid. You can test different combinations before discussing a loan offer with a lender.</p>
        <p className="text-sm leading-7 text-foreground-secondary">Atoolix uses the standard reducing-balance EMI formula with the annual rate converted to a monthly rate. A lender's actual schedule may differ because repayment frequency, rate resets, fees, insurance, taxes, rounding, disbursement timing, and lender-specific rules vary.</p>
      </section>
      <section className="space-y-4" aria-labelledby="home-loan-planning">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-border bg-card p-5"><h3 className="font-semibold text-foreground">Compare loan tenures</h3><p className="mt-2 text-sm leading-7 text-foreground-secondary">A longer tenure generally lowers the monthly EMI but keeps the principal outstanding for longer. Compare a comfortable EMI with the total interest cost before choosing a repayment period.</p></div>
          <div className="rounded-2xl border border-border bg-card p-5"><h3 className="font-semibold text-foreground">Test an interest-rate change</h3><p className="mt-2 text-sm leading-7 text-foreground-secondary">Compare the repayment impact of different rates. This is useful when evaluating loan offers or understanding how a rate change could affect an existing repayment plan.</p></div>
          <div className="rounded-2xl border border-border bg-card p-5"><h3 className="font-semibold text-foreground">Model a lump-sum prepayment</h3><p className="mt-2 text-sm leading-7 text-foreground-secondary">Enter an additional principal payment and compare the resulting interest and repayment period with the original schedule.</p></div>
          <div className="rounded-2xl border border-border bg-card p-5"><h3 className="font-semibold text-foreground">Compare EMI versus tenure reduction</h3><p className="mt-2 text-sm leading-7 text-foreground-secondary">After a prepayment, reducing the future EMI can improve monthly cash flow, while keeping the EMI higher can generally repay principal sooner. The lender's rules determine which option is available.</p></div>
        </div>
      </section>
      <section className="space-y-4" aria-labelledby="home-loan-example">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 text-sm leading-7 text-foreground-secondary">
      ...
      <section className="space-y-4" aria-labelledby="home-loan-faq">
        ...
        <div className="space-y-4 text-sm leading-7 text-foreground-secondary">
          <div><h3 className="font-semibold text-foreground">Does a longer home-loan tenure reduce total interest?</h3><p>No. It normally reduces the monthly EMI but increases total interest because the principal remains outstanding for longer.</p></div>
          <div><h3 className="font-semibold text-foreground">Does home-loan prepayment reduce interest?</h3><p>A principal prepayment can reduce future interest because it reduces the outstanding balance earlier. The exact saving depends on timing, amount, rate, remaining tenure, and lender rules.</p></div>
          <div><h3 className="font-semibold text-foreground">Should I reduce EMI or tenure after a prepayment?</h3><p>Keeping the EMI similar and shortening the tenure can increase interest savings, while reducing EMI provides more monthly cash-flow flexibility. Your lender's terms determine which option is available.</p></div>
          <div><h3 className="font-semibold text-foreground">Will this calculator exactly match my bank's schedule?</h3><p>Not necessarily. The calculator provides an estimate from your inputs. Fees, insurance, taxes, rate changes, rounding, disbursement timing, repayment frequency, and lender-specific rules can change the final repayment schedule.</p></div>
        </div>
      </section>
      <section className="space-y-4" aria-labelledby="related-loan-calculators">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <a href="/tools/calculator/emi-calculator" className="rounded-2xl border border-border bg-card p-5 hover:bg-surface-raised transition"><h3 className="font-semibold">EMI Calculator</h3><p className="mt-2 text-sm text-foreground-faint">Compare general loan repayment scenarios and switch between home, car, and personal loan calculations.</p></a>
          <a href="/tools/calculator/car-loan-emi-calculator" className="rounded-2xl border border-border bg-card p-5 hover:bg-surface-raised transition"><h3 className="font-semibold">Car Loan EMI Calculator</h3><p className="mt-2 text-sm text-foreground-faint">Estimate vehicle-loan EMI, total interest, and the effect of extra payments.</p></a>
          <a href="/tools/calculator/personal-loan-emi-calculator" className="rounded-2xl border border-border bg-card p-5 hover:bg-surface-raised transition"><h3 className="font-semibold">Personal Loan EMI Calculator</h3><p className="mt-2 text-sm text-foreground-faint">Calculate unsecured-loan EMI and compare rate and tenure scenarios.</p></a>
        </div>
      </section>
    </div>
  );
