...
export function InvestmentCalculatorGuide({ active }: InvestmentCalculatorGuideProps) {
  ...
  return (
    <section aria-labelledby="investment-calculator-guide-heading" className="space-y-5 rounded-3xl border border-border bg-card p-5 sm:p-6">
      <div className="space-y-2">
        <h2 id="investment-calculator-guide-heading" className="text-xl font-bold tracking-tight text-foreground sm:text-2xl">
        ...
        {activeCalculator && (
          <p className="max-w-3xl text-sm leading-7 text-foreground-secondary sm:text-base">
            <strong className="text-foreground">{activeCalculator.name}:</strong>{" "}
            {activeCalculator.intent}
          </p>
        )}
        <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground sm:text-base">
          If you need a different financial calculation, explore the full
          <Link href="/finance" className="ml-1 text-foreground underline decoration-border-strong underline-offset-4 hover:text-blue-300">
      ...
      {active === "sip" && (
        <div className="rounded-2xl border border-border bg-card p-4 sm:p-5" aria-labelledby="sip-example-heading">
          <h3 id="sip-example-heading" className="text-base font-semibold text-foreground sm:text-lg">
      ...
      {active === "lumpsum" && (
        <div className="rounded-2xl border border-border bg-card p-4 sm:p-5" aria-labelledby="lumpsum-example-heading">
          <h3 id="lumpsum-example-heading" className="text-base font-semibold text-foreground sm:text-lg">
      ...
      {active === "xirr" && (
        <div className="rounded-2xl border border-border bg-card p-4 sm:p-5" aria-labelledby="xirr-example-heading">
          <h3 id="xirr-example-heading" className="text-base font-semibold text-foreground sm:text-lg">
      ...
      {active === "cagr" && (
        <div className="rounded-2xl border border-border bg-card p-4 sm:p-5" aria-labelledby="cagr-example-heading">
          <h3 id="cagr-example-heading" className="text-base font-semibold text-foreground sm:text-lg">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {CALCULATORS.map((item) => (
          <Link key={item.id} href={item.href} className={`rounded-2xl border p-4 transition hover:border-blue-400/30 hover:bg-surface-raised ${item.id === active ? "border-blue-400/30 bg-blue-400/[0.08]" : "border-border bg-card"}`}>
      ...
      <div className="overflow-x-auto rounded-2xl border border-border">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-white/10 text-foreground-secondary">
            ...
            <tr><td className="px-4 py-3">I have multiple cash flows on specific dates</td><td className="px-4 py-3"><Link className="text-foreground underline underline-offset-4" href="/tools/calculator/xirr-calculator">XIRR Calculator</Link></td></tr>
          </tbody>
        </table>
      </div>
        ...
        <p className="text-sm leading-relaxed text-muted-foreground">
          Looking for a different return calculation? {visibleCalculators.map((item, index) => (
            <span key={item.id}>
              ...
              <Link href={item.href} className="text-foreground underline decoration-border-strong underline-offset-4 hover:text-blue-300">{item.name}</Link>
            </span>
