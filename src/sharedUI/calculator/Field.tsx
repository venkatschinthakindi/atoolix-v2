  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <input {...inputProps} id={fieldId} required={required} aria-invalid={Boolean(error)} className={`w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white outline-none transition focus:border-white/25 focus:ring-2 focus:ring-white/10 disabled:cursor-not-allowed disabled:opacity-50 ${suffix ? "pr-12" : ""} ${className}`.trim()} />
      ...
      {error ? <p role="alert" className="text-xs text-red-300">{error}</p> : hint ? <p className="text-xs text-white/50">{hint}</p> : null}
    </div>
  );
}
