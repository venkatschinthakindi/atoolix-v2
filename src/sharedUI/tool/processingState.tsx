...
export function ProcessingState({
  ...
  return (
    <div className={`rounded-2xl border border-border bg-card p-4 sm:p-5 ${className}`.trim()} aria-live="polite" aria-busy={status === "processing"}>
      <div className="flex items-center justify-between gap-3">
        <span className="text-sm font-medium text-foreground">{statusLabel}</span>
        {clampedProgress != null ? <span className="text-xs tabular-nums text-foreground-faint">{Math.round(clampedProgress)}%</span> : null}
      </div>
      {description ? <p className="mt-1 text-sm text-foreground-secondary">{description}</p> : null}
      {clampedProgress != null ? (
        <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-surface-raised" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={clampedProgress}>
