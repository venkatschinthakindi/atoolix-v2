...
export function ToolHeader({ title, description, icon: Icon, actions, className = "" }: ToolHeaderProps) {
  return (
    <header className={`flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between ${className}`.trim()}>
      <div className="flex min-w-0 items-start gap-3">
        {Icon ? (
          <div className="shrink-0 rounded-xl bg-white/5 p-2 text-blue-300" aria-hidden="true">
