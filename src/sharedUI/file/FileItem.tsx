...
export function FileItem({ file, selected = false, preview, metadata = true, removable = false, disabled = false, actions, onRemove, onSelect, className = "" }: FileItemProps) {
  ...
  return <li className={`flex min-w-0 items-center gap-3 rounded-xl border border-border bg-card p-3 transition ${selected ? "ring-2 ring-white/30" : ""} ${disabled ? "opacity-50" : ""} ${className}`.trim()} aria-selected={interactive ? selected : undefined}>
