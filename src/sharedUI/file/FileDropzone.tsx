...
export function FileDropzone({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`rounded-2xl border border-dashed p-5 text-center transition sm:p-8 ${
        dragging ? "border-white/50 bg-white/10" : "border-white/15 bg-white/[0.03]"
      } ${disabled ? "pointer-events-none opacity-50" : ""} ${className}`.trim()}
    >
      <input
        ref={inputRef}
        id={inputId}
      ...
      <label htmlFor={inputId} className="block cursor-pointer">
        ...
        <span className="mt-4 inline-flex rounded-xl bg-white px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-white/90">
          {browseLabel}
        </span>
      </label>
    </div>
