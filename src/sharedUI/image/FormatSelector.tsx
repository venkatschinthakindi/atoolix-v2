...
export function FormatSelector({ value, options, onChange, label = "Format", disabled = false, className = "" }: FormatSelectorProps) {
  return (
    <label className={`block space-y-1.5 ${className}`.trim()}>
      ...
      <select value={value} onChange={(e: ChangeEvent<HTMLSelectElement>) => onChange(e.target.value)} disabled={disabled} className="w-full rounded-xl border border-border bg-card px-3 py-2 text-sm text-foreground outline-none focus:border-blue-400 disabled:cursor-not-allowed disabled:opacity-50">
